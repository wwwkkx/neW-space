...
async function saveDocumentToNotion(userId: string, document: any) {
  try {
    ...
    const response = await fetch("https://api.notion.com/v1/pages", {
      ...
      body: JSON.stringify({
        ...
        properties: {
          ...
          标签: {
            multi_select: document.tags.map((tag: string) => ({ name: tag })),
          },
          优先级: {
            select: { name: document.priority },
          },
          创建日期: {
            date: { start: document.createdAt.split("T")[0] },
          },
          类型: {
            select: { name: "文档" },
          },
        ...
        children: [
          {
            ...
            type: "heading_2",
            heading_2: {
              rich_text: [{ type: "text", text: { content: "文档摘要" } }],
            },
          },
          {
            object: "block",
          ...
          },
          {
            ...
            type: "paragraph",
            paragraph: {
              rich_text: [{ type: "text", text: { content: document.originalContent } }],
            },
          },
        ],
      }),
    ...
    console.log("Successfully saved document to Notion")
  } catch (error) {
