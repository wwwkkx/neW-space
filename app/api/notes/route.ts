...
async function saveToNotion(userId: string, note: any) {
  try {
    ...
    const response = await fetch("https://api.notion.com/v1/pages", {
      ...
      body: JSON.stringify({
        ...
        properties: {
          ...
          创建日期: {
            date: { start: note.createdAt.split("T")[0] },
          },
          类型: {
            select: { name: "笔记" },
          },
        ...
        children: [
          ...
          },
          {
            ...
            type: "heading_2",
            heading_2: {
              rich_text: [{ type: "text", text: { content: "原始内容" } }],
            },
          },
          {
            object: "block",
            type: "paragraph",
            paragraph: {
              rich_text: [{ type: "text", text: { content: note.originalContent } }],
            },
          },
        ],
      }),
    ...
    console.log("Successfully saved note to Notion")
  } catch (error) {
